...
int check_input(char input[])
{
  ...
  else
  {
    ...
    int out_of_range = 0;
    for (int i = 0; i < 4; i++)
    { //checking if input contains 8 or 9
      digit = val % 10;
      val = val / 10;
      if (digit == 9 || digit == 8)
      {
...
int check_places(char secret_code[], char input[])
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    { // if it's not wellplaced, we want to throw the values in temp arrays
...
int main(int ac, char **av)
{
  ...
  for (int attempt = 0; attempt < 10; attempt++)
  {
    ...
    { //while input is incorrect, notify an user and ask new input
      printf("Wrong input!\n");
      read_input(input);
    } //ask and read an input
